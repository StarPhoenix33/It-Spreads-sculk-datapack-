...
execute if block ~ ~1 ~ #sculk:vegetation_replaceable positioned ~ ~1 ~ run function sculk:spread/vegetation_place
execute if predicate sculk:vegetation_place_chance if block ~ ~ ~ sculk if block ~ ~1 ~ #sculk:growth_replaceable positioned ~ ~1 ~ unless entity @n[tag=sculk,tag=!tendril,tag=!spike,distance=..2.5] run function sculk:spread/vegetation_place
